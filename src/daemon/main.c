...
int help(int exitcode) {
    ...
    fprintf(stream, "%s", "\n"
            ...
            " +----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+--->\n"
            "\n"
            " Copyright 2025 PulseNode Engine.\n"
            " Released under GNU General Public License v3 or later.\n"
            "\n"
            " Engine     : PulseNode Core\n"
            " Components : Metric Ingestion, Anomaly Scoring, Dashboarding\n"
            "\n"
            "\n"
    );
...
#define delta_startup_time(msg)                                     \
    do {                                                            \
        ...
        if(prev_msg) {                                              \
            netdata_log_info("PULSENODE STARTUP: in %7llu ms, %s - next: %s", (now_ut - last_ut) / USEC_PER_MS, prev_msg, msg); \
            LOG_TO_STDERR(" > startup: in %7llu ms, %s - next: %s\n", (now_ut - last_ut) / USEC_PER_MS, prev_msg, msg); \
        }                                                           \
        else {                                                      \
            netdata_log_info("PULSENODE STARTUP: next: %s", msg);     \
